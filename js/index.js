...
let CDmoney = $('#CDmoney');
let CDyear = $('#CDyear');
let CDbuyButton = $('#CDbuyButton');
let CDcompleteButton = $('#CDcompleteButton');
let CDcompleteyear = $('#CDcompleteyear');
let CDreleaseButton = $('#CDreleaseButton');
...
// 當按下購買定存按鍵時
CDbuyButton.on('click', async function () {
	...
	var a = document.getElementById("CDmoney").value ;//取得CDmoney欄位的輸入值
	var b = document.getElementById("CDyear").value ;
	$("#money").text("金額: " + a);//更新money欄位資料 後面加上方取得的CDmoney
  $("#year").text("期數: " + b);
  
	// 購買定存
	bank.methods.CDbuy(parseInt(CDyear.val(), 10)).send({
		from: nowAccount,
		gas: 3400000,
		value: web3.utils.toWei(CDmoney.val(), 'ether')
	})
		.on('receipt', function (receipt) {
			log(receipt.events.CDEvent.returnValues, '購買成功')
...
// 當按下合約期滿按鍵時
CDcompleteButton.on('click', async function () {
  ...
  $("#year").text("期數: ");
	// 合約期滿
	bank.methods.CDcomplete().send({
		from: nowAccount,
		gas: 3400000
	})
		.on('receipt', function (receipt) {
			log(receipt.events.CDcompleteEvent.returnValues, '期滿領回成功')
...
// 當按下提前解約按鍵時
CDreleaseButton.on('click', async function () {
  ...
  $("#year").text("期數: ");
	// 提前解約
	bank.methods.CDcancel(parseInt(CDcompleteyear.val(), 10)).send({
		from: nowAccount,
		gas: 3400000
	})
		.on('receipt', function (receipt) {
			log(receipt.events.CDcancelEvent.returnValues, '解約成功')
